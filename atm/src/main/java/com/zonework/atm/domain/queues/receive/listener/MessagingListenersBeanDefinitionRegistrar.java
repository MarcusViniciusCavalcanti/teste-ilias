...
import com.zonework.atm.domain.queues.receive.listener.rabbit.configuration.RabbitListenerBeanDefinitionRegistrar;
...
public class MessagingListenersBeanDefinitionRegistrar implements ImportBeanDefinitionRegistrar, EnvironmentAware {
	...
	public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
		var queuesReader = new EnvironmentArrayPropertiesReader(environment, PropertiesNames.QUEUES, PropertiesNames.QUEUE_NAME);

		while (queuesReader.hasNext()) {

			if (queuesReader.getProperty(PropertiesNames.SIMPLE_LISTENER_BEAN_ID) != null) {

				var connectionType = getConnectionType(environment, queuesReader);

				assertConnectionTypeSupportsListeners(connectionType, queuesReader);

				var listenerRegistrar = getListenerRegistrar(connectionType, queuesReader, registry);
	...
	private static ConnectionType getConnectionType(Environment environment,
													EnvironmentArrayPropertiesReader queuesReader) {
		var connectionType = queuesReader.getRequiredProperty(PropertiesNames.QUEUE_CONNECTION_TYPE, ConnectionType.class);
	...
	private static void validateConfigs(ConnectionType connectionType, Environment env) {
		var connectionTypeActivation = env.getProperty(connectionType.getActivationProperty());
